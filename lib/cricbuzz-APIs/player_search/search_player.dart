...
import 'package:cricketX/cricbuzz-APIs/player_stats/player_stats.dart';
...
class _SearchPlayerState extends State<SearchPlayer> {
  ...
  Future<void> fetchPlayerImage(List<dynamic>? playersList) async {
    ...
    try {
      for (int i = 0; i < playersList!.length; i++) {
        ...
        Uint8List? image = await ImageService.fetchImage(img.toString(), apiKey: apiKeyProvider.apiKey);
  ...
  Future<void> searchPlayer(String playerName) async {
    ...
    try {
    final url = Uri.parse('https://cricbuzz-cricket.p.rapidapi.com/stats/v1/player/search?plrN=$playerName');
